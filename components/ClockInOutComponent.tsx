import { View, Text, StyleSheet, TouchableOpacity, TextInput, ScrollView, Modal, Alert, Platform, Linking, KeyboardAvoidingView, ActivityIndicator } from 'react-native';
...
import { Clock, CheckCircle, Coffee, FileText, Calendar, MapPin, Hourglass, LogOut } from 'lucide-react-native';
...
async function getCurrentLocation(highAccuracy = false): Promise<GeoPoint> {
  if (Platform.OS === 'web') {
    ...
    const first = await tryGeo({ timeout: 10_000, maximumAge: 120_000, enableHighAccuracy: highAccuracy });
    if (first.latitude !== 0 || first.longitude !== 0) return first;

    // Second attempt: force a fresh position with a longer timeout.
    return tryGeo({ timeout: 20_000, maximumAge: 0, enableHighAccuracy: false });
  }
  ...
  try {
    ...
    if (locOrNull) return { latitude: locOrNull.coords.latitude, longitude: locOrNull.coords.longitude };
    ...
    if (highAccuracy) {
      ...
      if (fallback) return { latitude: fallback.coords.latitude, longitude: fallback.coords.longitude };
    }
...
export default function ClockInOutComponent({ projectId, projectName, officeRole, compact = false }: ClockInOutComponentProps) {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(`@clockloc_${currentEntry.id}`).then(val => {
      if (!val) return;
      try {
        const loc: GeoPoint = JSON.parse(val);
        if (loc.latitude !== 0 || loc.longitude !== 0) {
          lastKnownLocationRef.current = loc;
          console.log('[GPS] lastKnownLocation restored from storage for entry:', currentEntry.id.slice(0, 8));
        }
      } catch {}
    }).catch(() => {});
  }, [currentEntry?.id]);
  ...
  async function pushLiveLocation(entryId: string, projId: string | undefined, locStatus: 'working' | 'on_break') {
    if (!user?.id || !user?.companyId || isOfficeMode) return;
    const loc = await getCurrentLocation();
    if (loc.latitude === 0 && loc.longitude === 0) {
      console.warn('[GPS] pushLiveLocation GPS returned (0,0) — skipping push, lastKnownLocation unchanged');
      return;
    }
    lastKnownLocationRef.current = loc;
    AsyncStorage.setItem(`@clockloc_${entryId}`, JSON.stringify(loc)).catch(() => {});
    console.log('[GPS] lastKnownLocation updated via tracking push:', loc.latitude.toFixed(5), loc.longitude.toFixed(5));
    try {
  ...
  function startLocationTracking(entryId: string, projId: string | undefined) {
    ...
    locationIntervalRef.current = setInterval(() => {
      ...
      if (!entry || entry.clockOut) { stopLocationTracking(); return; }
      const locStatus = entry.lunchBreaks?.some(lb => !lb.endTime) ? 'on_break' : 'working';
  ...
  const completeClockIn = async () => {
    ...
    if (clockInLocation.latitude !== 0 || clockInLocation.longitude !== 0) {
      ...
      console.log('[GPS] lastKnownLocation updated at clock-in:', clockInLocation.latitude.toFixed(5), clockInLocation.longitude.toFixed(5));
    } else {
      console.warn('[GPS] clock-in GPS returned (0,0) — lastKnownLocation NOT updated');
    }
    ...
    try {
      ...
      if (clockInLocation.latitude !== 0 || clockInLocation.longitude !== 0) {
        AsyncStorage.setItem(`@clockloc_${savedEntry.id}`, JSON.stringify(clockInLocation)).catch(() => {});
      }
  ...
  const completeClockOut = async () => {
    ...
    console.log('[GPS] clock-out freshLocation:', freshLocation.latitude, freshLocation.longitude);
    console.log('[GPS] lastKnownLocationRef at clock-out:', lastKnownLocationRef.current);

    const freshIsValid = freshLocation.latitude !== 0 || freshLocation.longitude !== 0;
    let clockOutLocation: GeoPoint;
    if (freshIsValid) {
      clockOutLocation = freshLocation;
      console.log('[GPS] clock-out using fresh GPS:', clockOutLocation.latitude.toFixed(5), clockOutLocation.longitude.toFixed(5));
    } else if (lastKnownLocationRef.current) {
      clockOutLocation = lastKnownLocationRef.current;
      console.log('[GPS] clock-out using FALLBACK lastKnownLocation:', clockOutLocation.latitude, clockOutLocation.longitude);
    } else {
      ...
      try {
        const stored = await AsyncStorage.getItem(`@clockloc_${currentEntry.id}`);
        if (stored) {
          const loc: GeoPoint = JSON.parse(stored);
          if (loc.latitude !== 0 || loc.longitude !== 0) {
            clockOutLocation = loc;
            console.log('[GPS] clock-out using STORED fallback location:', loc.latitude, loc.longitude);
          } else {
            clockOutLocation = freshLocation;
          }
        } else {
      ...
      if (clockOutLocation.latitude === 0 && clockOutLocation.longitude === 0) {
        console.warn('[GPS] clock-out GPS (0,0) AND no cached location — clock-out will save WITHOUT location');
      }
    }
    ...
    if (effectiveRate > 0 && hoursWorked > 0 && projectId && !isOfficeMode) {
      ...
      if (existingExpense) {
        ...
        try {
          ...
          console.warn('[Clock Out] Labor expense save failed (non-fatal):', expenseErr);
        }
      }
    } else {
      if (!effectiveRate) {
  ...
  const handleLunchStart = async () => {
    ...
    if (startLocation.latitude !== 0 || startLocation.longitude !== 0) {
      ...
      AsyncStorage.setItem(`@clockloc_${currentEntry.id}`, JSON.stringify(startLocation)).catch(() => {});
      console.log('[GPS] lastKnownLocation updated at lunch-out:', startLocation.latitude.toFixed(5), startLocation.longitude.toFixed(5));
    } else {
      console.warn('[GPS] lunch-out GPS returned (0,0) — lastKnownLocation unchanged, current ref:', lastKnownLocationRef.current);
    }
    ...
    console.log(`[Lunch] ${user?.name} started lunch break at ${formatTime(new Date())} @ ${startLocation.latitude.toFixed(5)},${startLocation.longitude.toFixed(5)}`);
  ...
  const handleLunchEnd = async () => {
    ...
    if (endLocation.latitude !== 0 || endLocation.longitude !== 0) {
      ...
      AsyncStorage.setItem(`@clockloc_${currentEntry.id}`, JSON.stringify(endLocation)).catch(() => {});
      console.log('[GPS] lastKnownLocation updated at lunch-in:', endLocation.latitude.toFixed(5), endLocation.longitude.toFixed(5));
    } else {
      console.warn('[GPS] lunch-in GPS returned (0,0) — lastKnownLocation unchanged, current ref:', lastKnownLocationRef.current);
    }
    ...
    console.log(`[Lunch] ${user?.name} ended lunch break at ${formatTime(new Date())} @ ${endLocation.latitude.toFixed(5)},${endLocation.longitude.toFixed(5)}`);
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                ...
                <View style={styles.modalButtons}>
                  ...
                  <TouchableOpacity
                    style={[styles.confirmButton, isClockingOut && { opacity: 0.7 }]}
  ...
  return (
    <View style={styles.container}>
      ...
      {shiftEntry && (() => {
        ...
        const endMs = shiftEntry.clockOut ? new Date(shiftEntry.clockOut).getTime() : Date.now();
        ...
        if (shiftEntry.lunchBreaks) {
          shiftEntry.lunchBreaks.forEach(lunch => {
            ...
            const le = lunch.endTime ? new Date(lunch.endTime).getTime() : Date.now();
            lunchMs += Math.max(0, le - ls);
          });
        }

        const clockInLabel = formatTime(shiftEntry.clockIn);
        const clockOutLabel = shiftEntry.clockOut ? formatTime(shiftEntry.clockOut) : 'In Progress';
            ...
            <View style={styles.shiftSummaryHeader}>
              ...
              {inProgress && (
                <View style={styles.shiftSummaryBadge}>
                  ...
                  <Text style={styles.shiftSummaryBadgeText}>In Progress</Text>
                </View>
              )}
            </View>
            <View style={styles.shiftSummaryGrid}>
              <View style={styles.shiftSummaryItem}>
                <View style={[styles.shiftSummaryIconWrap, { backgroundColor: '#EFF6FF' }]}>
              ...
              <View style={styles.shiftSummaryItem}>
                <View style={[styles.shiftSummaryIconWrap, { backgroundColor: '#ECFDF5' }]}>
                  <LogOut size={16} color="#16A34A" />
                </View>
                <Text style={styles.shiftSummaryLabel}>Clocked Out</Text>
                <Text style={[styles.shiftSummaryValue, inProgress && styles.shiftSummaryValueMuted]}>
                  {clockOutLabel}
                </Text>
              </View>
              <View style={styles.shiftSummaryItem}>
                <View style={[styles.shiftSummaryIconWrap, { backgroundColor: '#FEF3C7' }]}>
                  <Coffee size={16} color="#D97706" />
                </View>
                <Text style={styles.shiftSummaryLabel}>Lunch Break</Text>
                <Text style={[styles.shiftSummaryValue, lunchMs === 0 && styles.shiftSummaryValueMuted]}>
                  {lunchLabel}
                </Text>
              </View>
              <View style={styles.shiftSummaryItem}>
                <View style={[styles.shiftSummaryIconWrap, { backgroundColor: '#EDE9FE' }]}>
      ...
      {shiftEntry && (() => {
        ...
        const events: { type: 'clock-in' | 'lunch-out' | 'lunch-in' | 'clock-out'; label: string; time: string | null; inProgress?: boolean }[] = [
        ...
        for (const b of normalizedBreaks) {
          events.push({ type: 'lunch-out', label: 'Lunch Out', time: b.startTime });
          if (b.endTime) {
            events.push({ type: 'lunch-in', label: 'Lunch In', time: b.endTime });
          } else {
            // Edge case: break in progress — Lunch Out without a matching Lunch In.
            events.push({ type: 'lunch-in', label: 'Lunch In', time: null, inProgress: true });
          }
        }

        if (shiftEntry.clockOut) {
          events.push({ type: 'clock-out', label: 'Clock Out', time: shiftEntry.clockOut });
        }
        ...
        const dotColor = (type: typeof events[number]['type']) => {
          switch (type) {
            case 'clock-in': return '#2563EB';
            case 'lunch-out': return '#D97706';
            case 'lunch-in': return '#F59E0B';
            case 'clock-out': return '#16A34A';
          }
        };
        ...
        return (
          <View style={styles.timeLogCard}>
            ...
            {events.map((ev, i) => (
              <View key={`${shiftEntry.id}-ev-${i}`} style={styles.timeLogRow}>
                <View style={[styles.timeLogDot, { backgroundColor: dotColor(ev.type) }]} />
                <Text style={styles.timeLogLabel}>{ev.label}</Text>
                {ev.inProgress || !ev.time ? (
                  <Text style={[styles.timeLogTime, styles.timeLogTimeMuted]}>In Progress</Text>
                ) : (
                  <Text style={styles.timeLogTime}>{formatTime(ev.time)}</Text>
                )}
              </View>
            ))}
          </View>
      ...
      {shiftEntry && (() => {
        ...
        return (
          <View style={styles.breakdownCard}>
            ...
            <View style={styles.breakdownRow}>
              <Text style={[styles.breakdownLabel, styles.breakdownLabelPaid]}>Paid Hours</Text>
          ...
          todayEntries.map((entry) => {
            ...
            const breaks = (entry.lunchBreaks ?? [])
              ...
              .map(lb => {
                ...
                return { startTime: lb.startTime, endTime: lb.endTime, startMs: s, endMs: e, inProgress: e === null };
              })
            ...
            const breakCountLabel =
              breaks.length === 0 ? '' : ` (${breaks.length} break${breaks.length === 1 ? '' : 's'})`;

      ...
      try {
        ...
        if (queryText.length > 10) {
          ...
          if (ragError) {
            ...
            ragChunks = chunks;
            console.log(`[AI Assistant] RAG: retrieved ${ragChunks.length} chunks (top: ${chunks[0]?.similarity?.toFixed(3)}) for: "${queryText.slice(0, 60)}"`);
          } else {
            console.log(`[AI Assistant] RAG: no chunks above threshold for: "${queryText.slice(0, 60)}"`);
          }
        }
      } catch (ragErr: any) {
